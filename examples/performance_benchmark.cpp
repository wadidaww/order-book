...
class PerformanceBenchmark {
  ...
  private:
    void benchmark_order_insertion() {
        ...
        std::cout << "  Inserted " << N << " orders in " << duration.count() / 1000.0 << " ms\n";
        std::cout << "  Average latency: " << std::fixed << std::setprecision(3) << avgLatency
                  << " μs\n";
        std::cout << "  Throughput: " << std::fixed << std::setprecision(0) << opsPerSec
                  << " ops/sec\n\n";
    }
    ...
    void benchmark_order_matching() {
        ...
        std::cout << "  Matched " << matchCount << " orders in " << duration.count() / 1000.0
                  << " ms\n";
    ...
    void benchmark_order_cancellation() {
        ...
        std::cout << "  Cancelled " << N / 2 << " orders in " << duration.count() / 1000.0
                  << " ms\n";
    ...
    void benchmark_market_data_access() {
        ...
        std::cout << "  " << N << " market data queries in " << duration.count() / 1'000'000.0
                  << " ms\n";
        std::cout << "  Average query latency: " << std::fixed << std::setprecision(0) << avgLatency
                  << " ns\n\n";
    }
    ...
    void benchmark_throughput() {
        ...
        book.setTradeCallback([&tradeCount](const Trade &) { tradeCount++; });
        ...
        std::cout << "  Processed " << N << " mixed operations in " << duration.count() << " ms\n";
        std::cout << "  Generated " << tradeCount << " trades\n";
        std::cout << "  Overall throughput: " << std::fixed << std::setprecision(0) << opsPerSec
                  << " ops/sec\n";
