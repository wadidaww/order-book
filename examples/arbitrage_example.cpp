...
void print_book(const std::string &name, const OrderBook &book) {
    ...
    if (bid && ask) {
        ...
        std::cout << std::fixed << std::setprecision(4) << bidPrice << " / " << askPrice;
    } else {
        std::cout << "No quotes";
    }
    std::cout << "\n";
...
int main() {
    ...
    if (opportunity) {
        ...
        std::cout << "Buy at Exchange " << (opportunity->buyBookIdx + 1) << " @ " << std::fixed
                  << std::setprecision(4) << buyPrice << "\n";
        std::cout << "Sell at Exchange " << (opportunity->sellBookIdx + 1) << " @ " << sellPrice
                  << "\n";
        ...
        std::cout << "Total potential profit: " << std::fixed << std::setprecision(2)
        ...
        bool buySuccess = buyBook->addOrder(1000, opportunity->buyPrice, execQty, Side::Buy);

        // Sell on expensive exchange
        bool sellSuccess = sellBook->addOrder(2000, opportunity->sellPrice, execQty, Side::Sell);
    ...
    if (opportunity) {
        ...
        std::cout << "Buy @ " << std::fixed << std::setprecision(4) << buyPrice << " on Exchange "
                  << (opportunity->buyBookIdx + 1) << "\n";
        std::cout << "Sell @ " << sellPrice << " on Exchange " << (opportunity->sellBookIdx + 1)
                  << "\n";
