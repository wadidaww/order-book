...
private:
    void benchmark_single_order_latency() {
        std::cout << "Benchmark: Single Order Add Latency (warm cache)" << std::endl;
        ...
        for (size_t i = 0; i < 1000; ++i) {
            book.add_order(i, Side::Buy, 10000 + i, 100);
        }
        ...
        for (size_t i = 0; i < iterations; ++i) {
            ...
            book.add_order(order_id++, Side::Buy, 10000 + (i % 100), 100);
    ...
    void benchmark_percentiles() {
        std::cout << "Benchmark: Latency Distribution (Mixed Operations)" << std::endl;
        ...
        for (size_t i = 0; i < iterations; ++i) {
            ...
            if (i % 3 == 0 && !active_orders.empty()) {
                ...
                Price price = side == Side::Buy ? (10000 - (i % 50)) : (10000 + (i % 50));
                book.add_order(order_id, side, price, 100);
    ...
    void print_latency_stats(const std::vector<int64_t>& latencies, const std::string& unit) {
        ...
        std::cout << "  Mean: " << std::fixed << std::setprecision(2) << mean << " " << unit << std::endl;
